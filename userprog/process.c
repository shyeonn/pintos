...
process_create_initd (const char *file_name) {
	char *fn_copy;
	//this is dummy pointer for strtok_r
	char *save_ptr;
	tid_t tid;
...
process_fork (const char *name, struct intr_frame *if_ UNUSED) {
	...
	uint64_t *arg[2] = {(uint64_t *)thread_current(),
		(uint64_t *)(if_)}; 
